...
goM :: Eq v => (v -> Maybe TmName) -> (v -> b -> Bool) -> TmF b v (M b v (Tm b Index)) -> M b v (Tm b Index)
goM isKnown isBound = \case
  TmFreeF a ->
    case isKnown a of
  ...
  TmLamF b mbody -> local (:|> b) mbody

namelessTerm :: Eq v => (v -> Maybe TmName) -> Tm v v -> Forall v (Tm v Index)
namelessTerm isKnown = runM . cata (goM isKnown (==))
...
mapAlphaTm = AlphaTm . cata go where
  go = \case
    TmFreeF i -> TmFree i
