...
import Searchterm.Interface.Core (ClsName, ConPat (..), Forall (..), Index (..), Inst (..),
                                  InstScheme (..), Partial (..), Pat (..), PatPair (..), Strained (..), Tm (..),
                                  Ty (..), TyF (..), TyScheme (..), TyVar (..), tySchemeBody, KindAnno (..), Kind (..), prettyScheme)
...
newtype TyFoundScheme = TyFoundScheme { unTyFoundScheme :: Forall (KindAnno TyUniq) (Strained Index (Ty Index)) }
  deriving stock (Show)
  deriving newtype (Eq, Ord)

instance Pretty TyFoundScheme where
  pretty = prettyScheme . unTyFoundScheme
...
fillTyScheme ws (TyFoundScheme s) = fmap TyScheme (namedStrained ws s)
...
insertStraintScheme (InstScheme (Forall ktvs (Strained cons inst))) = do
...
insertTyScheme onVar _ts@(TyScheme (Forall ktvs (Strained cons ty))) = do
...
funCtxFits goalKey = traceScopeM "Fun ctx fit" $ do
  guardDepth
  ctx <- asks envCtx
  -- Look at all terms in the context,
  choose (toListWithIndex ctx) $ \((_, candTyUniq), candIx) -> do
    -- and find their type,
    (_, candTyVal) <- lookupGoal candTyUniq
    case candTyVal of
      -- if it's a function type, see if applying it gets us anywhere!
      TyFunF candArgUniq candRetUniq -> do
        -- See if we can align the return type with the goal
        _ <- tryAlignTy goalKey candRetUniq
        -- If so, find an arg to call the function with
        candArgTm <- recSearchUniq candArgUniq
        -- and return an application of the function and arg
        pure (TmApp (TmFree candIx) candArgTm)
      _ -> empty
